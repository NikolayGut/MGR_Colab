...
from model_scatter_6_14 import model_audio
from dataloader_scatter_6_14 import datatype
...
def main():
    ...
    if args.resume:
        checkpoint = torch.load(Config["scatter_6_14"]["scatter_model_best"].format(ff))
    ...
    for epoch in range(0, epoches-start_epoch):
        ...
        with open(Config["scatter_6_14"]["scatter_train_process_txt"].format(ff), 'a') as f:
            f.write(line)
        dir = Config["scatter_6_14"]["scatter_checkpoint"].format(ff)
        ...
        if is_best:
            best_acc = test_acc
            dir_best = Config["scatter_6_14"]["scatter_model_best"].format(ff)
            print(dir_best)
...
def test(val_loader, model, criterion, criterion_cent, epoch, use_cuda):
    ...
    out_w = Config["scatter_6_14"]["save_result"].format(mode,ff)
    feat_w = Config["scatter_6_14"]["extract_feature"].format(mode,ff)
