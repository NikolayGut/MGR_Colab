...
def main():
    ...
    optimizer = optim.RMSprop(model.parameters(), lr=0.001, alpha=0.95, eps=1e-8, weight_decay=1e-5, momentum=0.95,
                              centered=True)
    ...
    for epoch in range(0, epoches-start_epoch):
        ...
        with open(Config["mel_6_14"]["mel_train_process_txt"].format(ff), 'a') as f:
            f.write(line)
        dir = Config["mel_6_14"]["mel_checkpoint"].format(ff)
        ...
        if is_best:
            ...
            dir_best = Config["mel_6_14"]["mel_model_best"].format(ff)
            print(dir_best)
