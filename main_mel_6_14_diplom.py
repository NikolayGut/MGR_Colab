...
from model_mel_6_14 import model_audio
from dataloader_mel_6_14 import datatype
...
def main():
    ...
    print(test_dataloader) # поменяно 24.11.23 
    ...
    #if args.resume:
    checkpoint = torch.load(Config["mel_6_14"]["mel_checkpoint"].format(ff))
    ...
    #if args.evaluate:
    print('\nEvaluation only')
    test_loss, test_acc, test_dict = test(test_dataloader, model, criterion, criterion_cent, start_epoch, use_cuda, 'test_dict_mel.csv')
    train_loss, train_acc, train_dict = test(train_dataloader, model, criterion, criterion_cent, start_epoch, use_cuda, 'train_dict_mel.csv')
    print('test_loss:{}, test_acc:{}'.format(test_loss, test_acc))
    print('train_loss:{}, train_acc:{}'.format(train_loss, train_acc))

    #output_to_csv(train_dict, 'train_dict.csv')
    #output_to_csv(test_dict, 'test_dict.csv')
